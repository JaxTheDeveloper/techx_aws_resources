...
resource "aws_network_acl" "vpc1_public" {
  ...
  # Allow HTTPS return traffic from internet (response from AWS services)
  ingress {
    rule_no    = 100
    action     = "allow"
    protocol   = "tcp"
  ...
  # Allow ephemeral return traffic from internet (response to NAT GW)
  ingress {
    rule_no    = 120
    action     = "allow"
    protocol   = "tcp"
  ...
  # Allow HTTPS from firewall subnet (traffic going through NAT GW)
  ingress {
    rule_no    = 130
    action     = "allow"
    protocol   = "tcp"
  ...
  # Allow HTTPS outbound to internet (via IGW)
  egress {
    rule_no    = 100
    action     = "allow"
    protocol   = "tcp"
  ...
  # Allow HTTP outbound to internet (via IGW)
  egress {
    rule_no    = 110
    action     = "allow"
    protocol   = "tcp"
  ...
  # Allow ephemeral ports outbound back to firewall subnet
  egress {
    rule_no    = 120
    action     = "allow"
    protocol   = "tcp"
...
resource "aws_network_acl" "vpc1_private_app" {
  ...
  # Allow ephemeral return traffic from VPC2 (database responses)
  ingress {
    rule_no    = 110
    action     = "allow"
    protocol   = "tcp"
  ...
  # Allow PostgreSQL outbound to VPC2 (RDS)
  egress {
    rule_no    = 90
    action     = "allow"
    protocol   = "tcp"
  ...
  # Allow NFS outbound to EFS in VPC1
  egress {
    rule_no    = 95
    action     = "allow"
    protocol   = "tcp"
...
resource "aws_network_acl" "vpc1_firewall" {
  ...
  # Allow ephemeral return traffic from public/NAT AZ1 subnet
  ingress {
    rule_no    = 140
    action     = "allow"
    protocol   = "tcp"
  ...
  # Allow ephemeral return traffic from public/NAT AZ2 subnet
  ingress {
    rule_no    = 150
    action     = "allow"
    protocol   = "tcp"
  ...
  # Allow HTTPS to public/NAT AZ1 subnet
  egress {
    rule_no    = 100
    action     = "allow"
    protocol   = "tcp"
  ...
  # Allow HTTPS to public/NAT AZ2 subnet
  egress {
    rule_no    = 110
    action     = "allow"
    protocol   = "tcp"
  ...
  # Allow HTTP to public/NAT AZ1 subnet
  egress {
    rule_no    = 120
    action     = "allow"
    protocol   = "tcp"
  ...
  # Allow HTTP to public/NAT AZ2 subnet
  egress {
    rule_no    = 130
    action     = "allow"
    protocol   = "tcp"
...
resource "aws_network_acl" "vpc2_isolated" {
  ...
  # Allow all TCP from VPC1 (app subnets via peering)
  ingress {
    rule_no    = 50
    action     = "allow"
    protocol   = "tcp"
