...
#   • Cross-VPC peering paths (Ingress→App, App→DB) carry real traffic.
...
#   ACCEPT :443 inbound from Ingress VPC CIDR  = VPC Link → Lambda ✓
#   ACCEPT :5432 outbound to DB VPC CIDR        = Lambda → RDS Proxy ✓
#   ACCEPT :443 outbound within App VPC          = Lambda → VPC Endpoints ✓
...
#   ACCEPT :5432 inbound from App VPC CIDR  = Lambda → RDS Proxy ✓
#   ACCEPT :443 outbound within DB VPC       = Proxy → Secrets Manager ✓
