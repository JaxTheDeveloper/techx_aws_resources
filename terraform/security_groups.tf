...
# Outbound: Port 5432 → RDS Proxy SG (Database VPC, via peering).
#           Port 443  → Interface VPC Endpoints within the Application VPC
...
# Lambda → RDS Proxy across the App↔DB VPC peering connection
resource "aws_security_group_rule" "lambda_egress_to_rds_proxy" {
  type              = "egress"
  description       = "Lambda → RDS Proxy (PostgreSQL) across App-to-DB VPC peering"
...
# Lambda → Interface VPC Endpoints inside the Application VPC (HTTPS)
resource "aws_security_group_rule" "lambda_egress_https_endpoints" {
  type              = "egress"
  description       = "Lambda → Interface VPC Endpoints (SNS, STS, Bedrock, CloudWatch Logs)"
  security_group_id = aws_security_group.lambda.id
...
# Outbound: Port 5432 → RDS PostgreSQL SG (same VPC, SG-reference safe).
#           Port 443  → Secrets Manager Interface Endpoint (DB VPC, CIDR).
# ──────────────────────────────────────────────────────────────────────────────
...
resource "aws_security_group_rule" "rds_proxy_egress_to_rds" {
  type                     = "egress"
  description              = "RDS Proxy → RDS PostgreSQL (native DB login)"
  security_group_id        = aws_security_group.rds_proxy.id
...
resource "aws_security_group_rule" "rds_proxy_egress_secrets_manager" {
  type              = "egress"
  description       = "RDS Proxy → Secrets Manager VPC Endpoint (fetch master password)"
  security_group_id = aws_security_group.rds_proxy.id
  cidr_blocks       = [var.db_vpc_cidr]
  protocol          = "tcp"
  from_port         = 443
