...
class DynamoDBUserStore:
    """pk=tenant_id, sk=DOC#<doc_id>. Matches dochub-w7-docs table schema."""
    ...
    def add_doc(self, tenant_id: str, doc_id: str, metadata: dict) -> None:
        self.table.put_item(Item={
            ...
            "sk": f"DOC#{doc_id}",
            "doc_id": doc_id,
            "uploaded_at": metadata.get("created_at", _now()),
            "created_at": _now(),
            **metadata,
        })
    ...
    def list_docs(self, tenant_id: str, doc_type: str | None = None) -> list:
        ...
        kwargs = {
            "KeyConditionExpression": Key("pk").eq(tenant_id) & Key("sk").begins_with("DOC#"),
        }
        if doc_type:
            kwargs["FilterExpression"] = Attr("doc_type").eq(doc_type)
        return self.table.query(**kwargs).get("Items", [])

    def get_doc(self, tenant_id: str, doc_id: str) -> dict | None:
        resp = self.table.get_item(Key={"pk": tenant_id, "sk": f"DOC#{doc_id}"})
