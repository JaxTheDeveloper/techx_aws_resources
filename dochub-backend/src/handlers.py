...
def _get_cw_client():
    """Trả về CloudWatch client, khởi tạo lần đầu khi thực sự cần dùng."""
    global _cw_client
...
def _extract_named_docs(question: str) -> list:
    ...
    Matches underscore-separated words (e.g. w7_project_announcement)
    optionally followed by a file extension (e.g. .md, .txt, .pdf).
    Skips UUID-like patterns and very short tokens.
    """
    names = set()
    q = question.lower()
...
def handle_query(
    ...
    ai_client,
    vector_store,
    vector_backend: str,  # kept for backward compat
    bedrock_kb_id: str,   # kept for backward compat
) -> dict:
    ...
    try:
        ...
        context = "\n\n".join(
            f"[{i+1}] (doc={c['metadata'].get('filename', c['doc_id'])} v{c['metadata'].get('version', 1)}) {c['text']}"
