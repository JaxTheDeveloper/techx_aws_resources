...
#
# Inbound:
#   443        — API calls arriving from the Ingress VPC (VPC Link to Lambda)
#   1024-65535 — Ephemeral return traffic from Database VPC (DB query results)
#                and from Interface VPC Endpoints within this VPC
#
# Outbound:
#   443        — Lambda to Interface VPC Endpoints (SNS, STS, Bedrock, Logs)
#   5432       — Lambda to RDS Proxy in Database VPC
...
# Lambda to Interface VPC Endpoints (SNS, STS, Bedrock Runtime, CloudWatch Logs)
...
# Lambda to RDS Proxy in the Database VPC
...
#
# Outbound:
#   443        — RDS Proxy to Secrets Manager VPC Endpoint (fetch DB password)
#   5432       — RDS Proxy to RDS instance (cross-AZ failover path)
...
# RDS Proxy to Secrets Manager Interface Endpoint (within Database VPC)
...
# RDS Proxy to RDS instance (intra-VPC, cross-AZ failover path)
